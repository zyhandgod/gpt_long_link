...
// @author       Miku-Y
// @version      1.0.2
// @description  在 ChatGPT 页面显示快捷支付、Token 和群聊入口，并兼容当前站点的翻译告警。
...
(() => {
    ...
    const QQ_LINK = "https://qm.qq.com/q/ISw5rllXEu";
    ...
    const STYLE = `
        ...
        #gopay-container.gopay-collapsed .gopay-body,
        #gopay-container.gopay-collapsed .gopay-footer { display: none; }
        ...
        .gopay-chat { color: #351c00; background: linear-gradient(135deg,#ffe3bd,#ffc36e); }
        ...
        .gopay-footer { padding: 12px 14px 14px; background: #efe6d6; border-top: 1px solid rgba(77,63,36,0.1); }
        .gopay-f-label { display: flex; justify-content: space-between; align-items: center; margin-bottom: 9px; color: rgba(24,21,18,0.5); font: 750 11px -apple-system,BlinkMacSystemFont,'Segoe UI',sans-serif; letter-spacing: 0; }
        .gopay-f-label::after { content: ""; width: 36px; height: 1px; background: rgba(77,63,36,0.18); }
        .gopay-link-list { display: grid; grid-template-columns: repeat(3, minmax(0, 1fr)); gap: 7px; }
        .gopay-link { min-width: 0; border: 1px solid rgba(77,63,36,0.12); border-radius: 12px; color: rgba(24,21,18,0.76) !important; background: rgba(255,250,241,0.74); text-decoration: none; font-size: 11px; font-weight: 760; padding: 8px 6px; text-align: center; transition: all 0.18s ease; white-space: nowrap; overflow: hidden; text-overflow: ellipsis; }
        .gopay-link:hover { color: #061a12 !important; background: #dff7e8; border-color: rgba(25,185,120,0.32); transform: translateY(-1px); }
    ...
    function showButton() {
        ...
        head.innerHTML = `
            ...
            <div class="gopay-subtitle">支付、Token、群聊入口集中处理</div>
        `;
        ...
        const qqBtn = makeRow(null, "💬", "群聊", "打开 QQ 群", "gopay-chat");
        qqBtn.addEventListener("click", () => window.open(QQ_LINK, "_blank"));

        const footer = document.createElement("div");
        footer.className = "gopay-footer";
        footer.innerHTML = `<span class="gopay-f-label">友链入口</span>`;

        const linkList = document.createElement("div");
        linkList.className = "gopay-link-list";
        [
            ["988.chat", "https://988.chat"],
            ["778.chat", "https://778.chat"],
            ["fk.ithte.com", "https://fk.ithte.com"],
        ].forEach(([text, url]) => {
            const a = document.createElement("a");
            a.className = "gopay-link";
            a.href = url;
            a.target = "_blank";
            a.textContent = text;
